...
Feature: Column PROCESSED in CONTRACT_DISPATCHER_SOURCE change to be 'N' when STATUS in CONTRACT_DISPATCHER_CLAIM is RELEASE
  ...
  @tag1
  Scenario: Claim Contract TWO WHEELERS in Contract Dispatcher Menu then idle for 10 minutes, STATUS in Table CONTRACT_DISPATCHER_CLAIM turn RELEASE and PROCESSED in CONTRACT_DISPATCHER_SOURCE turns N
    Given Go to IST
    ...
    And Exec Query idle for Ten Minutes "SELECT * FROM CONTRACT_DISPATCHER_SOURCE WHERE TEXT_CONTRACT_NUMBER='332'"
    And Select Menu "Contract Dispatcher" 