...
@tag
Feature: Contract Dispatcher Two Wheelers EFP
  I want to use this template for my feature file

  @tag1
  Scenario: Assign User to Contract Two Wheelers EFP
    Given Go to IST
  ...
  Scenario: Select Menu Contract Dispatcher and Select Contract based on type contract Two Wheelers EFP
    Given Go to IST
    When Login IST with valid credentials
      | username    | password                 |
      | mobile.test | heMxIzEAyQpycx257HePHg== |
    ...
    And Type on textarea note after based on mistake name "KTP / Resi - Alamat tetap pemohon pada sistem (HoS) berbeda dengan KTP" with value "TESTING after"
