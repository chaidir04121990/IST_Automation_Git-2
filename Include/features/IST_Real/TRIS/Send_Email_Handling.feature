...
Feature: Send Email Handling
  ...
  Scenario: Send Email by Select Menu Email Handling from Invoice System then Generate Email Handling
    ...
    And Type EmailClient on "Alamat Email yang digunakan client" with value "chaidir.pratama01@homecredit.co.id"
    And Click button "Generate Email"
    And Verify Email Handling
    And Close browser

  #Scenario: Check email sent
    #Given Go to Outlook
    #When Login Outlook with valid credentials
    #Then Login Outlook Succeed
    #And Verify Email Handling TRIS Sent
