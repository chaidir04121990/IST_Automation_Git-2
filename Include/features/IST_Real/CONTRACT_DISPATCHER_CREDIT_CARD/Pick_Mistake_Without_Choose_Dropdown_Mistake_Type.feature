...
Feature: Pick Mistake without choose dropdown mistake type
  ...
  @tag1
  Scenario: Assign User to Contract Credit Card
    Given Go to IST
    ...
    And Check for username if already assigned to "Credit Card"
    And Wait page for load
    And Click button "Logout"
    And Close browser
    
   Scenario: Select Menu Contract Dispatcher and Select Contract based on type contract Credit Card
    Given Go to IST
    When Login IST with valid credentials
      | username    | password                 |
      | mobile.test | heMxIzEAyQpycx257HePHg== |
    ...
    And Verify Tab Matrix for "CREDIT_CARD"
