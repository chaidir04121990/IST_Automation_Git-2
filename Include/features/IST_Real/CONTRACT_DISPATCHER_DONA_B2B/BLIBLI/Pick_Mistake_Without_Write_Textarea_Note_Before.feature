...
Feature: Pick Mistake Without Write Textarea Note_before
  ...
  Scenario: Assign User to Contract Dona BLIBLI
    ...
    Scenario: Select Menu Contract Dispatcher and Select Contract based on type contract DONA BLIBLI
    Given Go to IST
    When Login IST with valid credentials
      | username    | password                 |
      | mobile.test | heMxIzEAyQpycx257HePHg== |
    ...
    And Type on textarea note after based on mistake name "20. KTP atau Resi - Alamat tetap pemohon pada sistem (HoS) berbeda dengan KTP" with value "Testing"
